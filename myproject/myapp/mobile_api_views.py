    ...
    if 'farmer_name' in keys and 'officer_id' in keys:
        ...
        if str(survey_view).lower() == 'true':
            ...
            for s in page_obj:
                ...
                surveys_data.append({
                    ...
                    'factory_name': s.factory_name or '-',
                    'plot_id': s.plot.id if s.plot else None,
                    'plot_name': s.plot_name or (s.plot.plot_code if s.plot else '-'),
        ...
        for p in all_plots:
            ...
            plots_by_village[vid].append({
                ...
                'image_url': request.build_absolute_uri(f'/media/{p.plot_code}.png') if p.plot_code else None,
            })
        ...
        if str(survey_view).lower() == 'true':
            ...
            for s in surveys:
                surveys_data.append({
                    ...
                    'plot_code': s.plot.plot_code if s.plot else '-',
                    'farmer_name': s.plot.farmer.name if s.plot and s.plot.farmer else '-',
                    'group_id': s.plot.group_id if s.plot else None,
                    'group_name': s.plot.group_name if s.plot else '-',
                    'factory_id': s.plot.factory_id if s.plot else None,
                    'factory_name': s.plot.factory_name if s.plot else '-',
                    'plot_id': s.plot.id if s.plot else None,
                    'plot_name': s.plot.plot_code if s.plot else '-',
                    'survey_stage': s.survey_stage or '-',
                    'survey_month': s.survey_month or '-',
                    'number_of_days': s.number_of_days,
                    'allocated_dates': s.allocated_dates or [],
